...
export default function Products({ user, products, totalPages, search, minValue, maxValue, sortBy, category, page }) {
  ...
  function handlePageSelect(data) {
    ...
    router.push(`/products?${searchParam + pageParam}`);
    setVisible(false);
  }
...
Products.getInitialProps = async ({ query: { search, minValue, maxValue, sortBy, category, page } }) => {
